...
export interface StageColors {
   ...
   * 當預設值，讓使用者能透過半透明看穿疊圖；PDF 分層匯出的白墨那一輪則透過 pdfLayerColors
   * 覆寫成 1——印刷分色版的白墨必須是實墨，不能是畫面用的網點，否則白墨會變薄，甚至被 RIP
   * 當成半調網目處理。
   */
  underprintOpacity?: number;
}

/** 畫面即時畫布使用的白墨不透明度；只在元件端當預設值使用，PDF 匯出不會沿用它 */
export const DEFAULT_UNDERPRINT_OPACITY = 0.8;

/** PDF 分層匯出時白墨要覆寫成的顏色：印刷分色版的慣例是「有墨處為黑」 */
export const UNDERPRINT_PDF_COLOR = '#000000';

/**
 * PDF 分層匯出時，依這一輪要輸出的 kind 決定暫存區截圖用的顏色／不透明度。
 * 只有白墨那一輪需要覆寫：顏色改成純黑、不透明度改成 1（實墨），不是畫面用的 0.8 網點；
...
export function pdfLayerColors(kind: ImpositionLayerKind, colors: StageColors): StageColors {
  return kind === 'underprint' ? { ...colors, underprint: UNDERPRINT_PDF_COLOR, underprintOpacity: 1 } : colors;
}
